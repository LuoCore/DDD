...
namespace Domain.CommandHandlers
{
    ...
    public sealed class InMemoryBus : IMediatorHandler
    {
        ...
        // 事件仓储服务
        private readonly Domain.Interface.IRepository.IEventStoreRepository _eventStoreService;


        public InMemoryBus(IMediator mediator, ServiceFactory serviceFactory, Domain.Interface.IRepository.IEventStoreRepository eventStoreService)
        {
            _mediator = mediator;
        ...
        /// <returns></returns>
        public Task RaiseEvent<T>(T @event) where T : IEventBase
        {
            // 除了领域通知以外的事件都保存下来
            if (!@event.MessageType.Equals("DomainNotification"))
                _eventStoreService?.Store(@event);
